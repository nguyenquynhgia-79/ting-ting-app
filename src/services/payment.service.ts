...
export class PaymentService {
  ...
  async createPayment(data: {
    group_id: string;
    payer_id: string; // The one who is paying back debt
    payee_id: string; // The one who is receiving money
    amount: number;
  }) {
    ...
    if (members.length < 2) {
      throw new AppError("One or both users are not members of this group", 400);
    }
  ...
  async confirmPayment(paymentId: string, userId: string) {
    ...
    if (payment.payee_id !== userId) {
      throw new AppError("Only the recipient can confirm this payment", 403);
    }
  ...
  async rejectPayment(paymentId: string, userId: string) {
    ...
    if (payment.payee_id !== userId) {
      throw new AppError("Only the recipient can reject this payment", 403);
    }
  ...
  async getSimplifiedDebts(groupId: string) {
    ...
    const debtors: { id: string; username: string; balance: number }[] = [];
    const creditors: { 
      id: string; 
      username: string; 
      balance: number;
    ...
    members.forEach((m) => {
      ...
      if (bal < -0.01) {
        ...
        creditors.push({ 
          ...
          account_number: m.user.account_number ? decrypt(m.user.account_number) : null,
